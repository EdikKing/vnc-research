...
> 本文档横向对比 3 种部署方式:**本机直装 / Docker 容器化 / systemd 服务化**。本文档**不下结论**,**操作员**和朋友根据自己场景挑。
...
> **本节是操作员的明确推荐,不是中立对比。如果你想看中立对比和条件分支,跳到下面"## 选择建议"章节。**

### 一句话推荐

**优先选方式一(本机直装),除非你有特定场景必须用方式二或方式三。**

### 为什么推荐方式一

vnc-research 仓库当前现状决定了方式一是阻力最小的选择:

- **开箱即用**:仓库自带 `scripts/start-all.sh`,里头已经写好 Xvfb / x11vnc / chromium 启动逻辑 + 环境自检 + 日志路径,直接 `apt install` + 跑脚本就能跑
- **零额外抽象**:没有 Dockerfile 模板(得自己写),没有 systemd unit 模板(得自己写),这两个方式都得你额外补工作量
- **调试最直观**:所有进程跑在宿主机,出问题直接 `ps aux | grep -E 'Xvfb|chromium'`,`tail -f /tmp/vnc-research-logs/*.log` 就能定位
- **跟 docs/00-快速开始.md 完全对齐**:5 分钟跑起来的所有步骤都是方式一的预设路径

### 三种方式适用场景速查

| 场景 | 推荐方式 |
|---|---|
| 个人开发机 / 学习原理 / 一次性测试 | **方式一本机直装** |
| CI 流水线 / 跨环境一致 / 多实例隔离 | 方式二 Docker |
| 生产环境长期运行 / 开机自启 / 进程守护 | 方式三 systemd |

### 何时升级到方式二或三

一旦你遇到下面的需求,就是该升级的信号:

- 想 `git push` 到 CI 自动跑 → **Docker**(把环境做成镜像,流水线一份)
- 服务器经常重启,要崩了自动拉起 → **systemd**(`Restart=on-failure`)
- 团队多人协作,每人环境不一致 → **Docker**(同一份镜像)
- 想接 ELK / Loki 日志系统 → **systemd**(`journalctl` 原生集成)

> 升级不是替换 —— 见下面"## 选择建议"末尾的"方式不互斥"说明,开发用方式一 + 部署用方式三是常见组合。
